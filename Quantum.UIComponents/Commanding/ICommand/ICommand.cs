...
namespace Quantum.Command
{
    ...
    /// <summary>
    /// Provides the basic contract for a global command.<para/>
    /// Global commands are application-wide parameterless commands processed by the MainWindow of the application. 
    ...
    ///                     This metadata type does not support multiple instances in the metadata collection of a global command.<para/>
    ...
    /// </summary>
    public interface IGlobalCommand : IStaticMetadataOwnerCommand<CommandMetadataCollection, ICommandMetadata>
    {
    }


    /// <summary>
    /// Provides the basic contract for a multi global command. <para/>
    /// Multi global commands represent a collection of application-wide commands provided by a getter delegate which are processed by the 
    ...
    ///                          This metadata type does not support multiple instances in the metadata collection of a multi global command. <para/>
    ...
    ///                            of the application's container. The current set of commmands associated with this MultiGlobalCommand will be wiped out, 
    ...
    ///                                this MultiGlobalCommand will be wiped out, and the getter delegate will be used to re-compute the commands. 
    ...
    /// </summary>
    public interface IMultiGlobalCommand : IMultiStaticMetadataOwnerCommand<ISubCommand, MultiCommandMetadataCollection, IMultiCommandMetadata>
    {
    }
    ...
    /// Subcommands are application-wide commads that are part of a set of commands return by a MultiGlobalCommand's getter delegate which
    /// are processed by the main window of the application depending on the parent MultiGlobalCommand's metadata and their own individual metadata.
    /// The parent MultiGlobalCommand must be defined as a property in a command container registered in the command manager service of the framework.
    /// After the parent MultiGlobalCommand gets registered and processed by the framework, the set of subcommands is computed and each of them is processed 
    ...
    ///                        on the parent MultiGlobalCommand's MultiMainMenuOption metadata (if the parent MultiGlobalCommand does not have a 
    ///                        MultiMainMenuOption metadata defined, this metadata type does nothing) and the order in the set of commands returned by 
    ///                        the parent MultiGlobalCommand's getter delegate. The other configuration options can be defined in the subcommand's 
